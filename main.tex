...
\subsection{Case-study Eglantine}
...
\end{equation}
The depth of the geothermal wells is found in the pre-studies given in the call for tender:
\begin{equation}
L_{gtw} = 267
\end{equation}
The mean temperature in the geothermal well can then be calculated:
\begin{equation}
T_{g, mean} = 15 \si{\celsius}
\end{equation}
\todo{add images of gradient and surface temperature from PGG report?}

It is assumed that the geothermal wells are well sized, in order to respect the natural recharge rate. The recharge rate is strongly dependent on the type of soil and is very important to ensure a sustainable use of the ground heat throughout the years. \\

\todo{Assume a dt of x C around the well? dynamic curve of use}


To determine the minimum approach temperature in the well, follow procedure described in Section \ref{ss:dtmin} using the following values:

The exchange area can be estimated, taking a typical diameter of diameter  = 32 mm\cite{siaSIA384Sondes2010, kruseStatusDevelopmentResearcha}\\
cylinder lateral surface\\

The overall heat transfer coefficient of the well, including the fluid, the bore wall and the soil, is assumed to be $9.3 kW/m^2K$ for a brine loop and $17.1 kW/m^2K$ for the CO2 direct expansion\cite{kruseStatusDevelopmentResearcha}.\\

\todo{this paragraph here or in values?}
The price for boreholes in Switzerland is found to be around $c_{wells} = 80$ [CHF/m] \cite{bawos.chMitErdsondenbohrungenKosten2018}. In order to provide the model with an energy dependent cost function, this value is transformed with help of the data from Table \ref{tab:ppa_prestudy}:
\begin{align}
...
result dTmin water ground = 14 and dtmin co2ground = 6.8.
This value is similar to values found in literature \cite{siaSIA384Sondes2010, lamarcheReviewMethodsEvaluate2010}\\

The water drop in the ground loop is assumed to be 4 \si{\celsius}$dT_{water}$ \cite{lundDESIGNCLOSEDLOOPGEOTHERMAL}
...
\todo{rewrite chapter}
To show the difference in performance between the CO2 (higher \textit{h}) and conventional refrigerants:...

\begin{itemize}
	\item maintain area with lower dT (lower $OC_{hp}$)
	\item same dT and lower area (lower $IC_{ex}$)
\end{itemize}

The optimization is done on water/refrigerant hp, since values well known (ic, Q, COP). 

CO2 $\Delta T_{min}$ is then calculated maintaining same Area as for ref hp. equation solved on \textit{Matlab} with help of function \textit{solve(equation \ref{eq:HEX_area}, $\Delta T_{min}$)}.

Thus following $\Delta T_{min}$ are have been calculated and used for heat exchanges in the model, shown in Table \ref{tab:dtmins}.
...
As discussed in Section \ref{ss:dx}, there is the possibility to directly expand the CO2 into the geothermal well, instead of exchanging with help of a secondary water loop. 

Results of simulation, show the difference in performance, \ref{tab:dxVSsl}.

\input{Tables/cp_comparison.tex}
\todo{complete table with exergy efficiencies}

CO2 heat pump has lower efficiencies (see Table \ref{tab:eta_comp}) thus $\eta_{COP}$ but much lower temperature difference to cover.

So much higher COP and exergy efficiency!
...
this results in higher OC. The main advantage can be the reduction of upfront costs, by avoiding geothermal drilling. Pressure losses as well as the energy for pumping have been neglected, so the cost of the water pipes is mainly dependent on the distance of the lake water. Thus the threshold distance can be calculated, for the choice between lake or geothermal:

\missingfigure{distLake threshold in TC/OC/IC graph}

A sensitivity analysis has been performed on this threshold, in function of the ground temperature, since it is a crucial parameter for different climates.

\missingfigure{distLake threshold (Tground)}
...
Geothermal model: In Switzerland this would correspond to different well depth. To study this thoroughly it would be necessary to have a model for the geothermal well, taking into account the temperature gradient in function of depth, as well as the varying heat transfer coefficient for the CO2 vapor on the upward pipe. This model should also account for the recharge rate of the soil, which would improve the equipment sizing. And thus storage, or gain through restoring/injecting heat.\\
